...
const Hero = () => {
  ...
  return (
    <section 
      ref={containerRef} 
      id="home" 
      className="relative min-h-screen flex items-center pt-24 sm:pt-28 md:pt-32 pb-12 sm:pb-16 md:pb-20 overflow-hidden"
    >
      <div className="max-w-7xl mx-auto px-5 sm:px-8 md:px-12 grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-center">
        ...
        <div className="lg:col-span-7 space-y-8 md:space-y-10 z-10">
          ...
          >
            ...
            <h1 className="text-4xl sm:text-6xl md:text-8xl lg:text-[120px] font-serif font-black leading-[0.85] sm:leading-[0.8] tracking-tighter text-white">
              <span className="block italic">{heroData.firstName}</span>
              <span className="block text-accent-indigo ml-2 sm:ml-4 md:ml-12">{heroData.lastName}</span>
            </h1>

            {/* Tagline */}
            <p className="max-w-xl text-sm sm:text-base text-slate-400 leading-relaxed font-light whitespace-pre-line">
              {heroData.tagline}
            </p>
        ...
        >
          <div className="relative aspect-[4/5] rounded-[2rem] overflow-hidden grayscale-0 lg:grayscale lg:hover:grayscale-0 transition-all duration-700 shadow-2xl border border-white/5">
            ...
            {/* Name Label */}
            <div className="absolute bottom-6 left-6 right-6 sm:bottom-10 sm:left-10 sm:right-10 p-4 bg-dark-900/40 backdrop-blur-xl border border-white/10 rounded-2xl flex items-center justify-center">
      ...
      <Motion.div 
        ...
        animate={{ opacity: 1 }}
        transition={{ delay: 1 }}
        className="absolute bottom-6 left-5 sm:bottom-12 sm:left-12 flex items-center gap-4"
      >
